...
### 新增：日记页 `/diary/`（按年 → 月 → 列表翻看）

站主要一个只收录日记、需要口令、按年月翻看的入口。

- **入口**：`yuuu.love/diary`，和 `/private/` 一样**不列入导航**（1a）；
  页脚另有一个不显眼的链接（1c）—— 不想要删掉 `Footer.astro` 里那一行即可。
- **口令**：复用私人角落那套（Worker 的 `/hidden`），**同一个 sessionStorage 键**
  （`yuuu-private`），所以 `/private/`、`/admin/*` 任一登过，这边不用再输。
- **三个视图**都在这一页里，位置记在 hash 上（`/diary/#2026/08`）：
  年（从 **2024** 起，倒序）→ 月（固定 1–12 全部显示，带篇数）→ 该月列表（日期+标题，倒序）。
  后退键能用、单级可收藏。
- **不做任何自动生成**：列表里只有真实写过的；空月份显示「还没有」。
- **一天一篇**：月份视图里点「✍ 写这一天」→ 出现该月日期格（已写过的标金色）；
  点已写过的日子 → 直接打开那一篇并提示「那天已经写过一篇」；
  点空白日子 → 跳到手机写作页并**预填分类=日记 + 该日期**。
  写作页那边也做了兜底：若同一天已有日记，直接打开已有的那篇。

### 为什么不做成三个静态页（重要）

**`dist/` 下的一切都是公开可下载的。** 静态页会把日记标题写进公开 HTML，
口令就形同虚设 —— 跟之前那个泄漏是同一类问题。
所以公开的只有**空壳 + 口令框**，清单在验口令后由 Worker 返回。
为此加了 audit **8.7**：拿全部文章的标题去 grep `diary/` 与 `private/` 的产物 HTML，
一个都不许命中（现在 18 个标题零泄漏）。

### 其它

- 新增 `tools/check-diary.mjs`（真浏览器测口令 → 年 → 月 → 列表 + 一天一篇），
  并进 `check:all`。现在一共 **11 项**。
- 手机写作页支持 `?cat=<分类>&date=<日期>` 预填（原只有 `?edit=<slug>`）。

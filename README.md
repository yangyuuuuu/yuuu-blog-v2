...
打开 **`https://yuuu.love/admin/g/`**，同一个口令。能做：

- 网格看所有图片，按**分类**筛选、按文件名搜
- 点一张图 → **复制路径**（写文章时直接粘到 Markdown 里）、**换分类**、看原图、删除
- 右下角 **＋** 上传（可多选）→ 先选分类 → 自动压缩后上传

> **分类怎么存的**：分类就是 `public/uploads/` 下的**文件夹名**，比如
> `/uploads/表情包/cake.webp` 归属「表情包」。所以你在 Decap 自带的媒体库里
> 也能看到同样的文件夹结构；已经在根目录的老图片显示为「未分类」，不用特意整理。
>
> 上传前会自动压缩（最长边 1600、转 JPEG）—— 手机上拍的照片动辄 5MB，
> 直接提交进 git 仓库会让仓库迅速变大、构建变慢。
...
> **正文没有长度要求**（原来卡 20 字，已按要求去掉）。短句、一张图配一行字、
> 一句话日记都可以直接发。唯一必需的是**标题** —— 没有标题就没有文件名。
>
> **关于换行**：Markdown 里**单个换行不算换行**，会被合并成同一段。
> 所以：
> - 在**手机写作页**（`/admin/m/`）里，默认开着「回车即换行」，
>   你按回车就是真的换行（保存时会写成 Markdown 硬换行，行尾两个空格）。
>   想要标准 Markdown 行为（空一行才换段）可以在「更多设置」里关掉。
> - 在 Decap 后台或直接改文件时是**标准 Markdown**：
>   想让两行分开显示，要么中间空一行，要么在行尾打两个空格。

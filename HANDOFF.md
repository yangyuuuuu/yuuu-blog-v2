...
### 9.25 ★ 部署前必须先 pull —— 否则会把后台新加的图从线上挤掉

**Cloudflare Pages 每次部署都是完整快照**：你发哪个 dist，线上就是哪个 dist。
而 **`/admin/g/` 和 `/admin` 上传图片是直连 GitHub API 的，本地仓库不会自动更新**。
两者一叠加就是事故：本地没先 `pull` 就 `build` + 部署 →
那些通过后台刚加的图片**不在你的 dist 里** → 线上文件消失（用户看到 404 / 空白卡片）。

我真踩了一次：用户上传的 `uploads/celeste/xxx.jpg` 因为我的部署从线上消失了。
（当时我一度以为是「自定义域传播延迟」—— 传播延迟确实存在，约 40~60 秒，
但那次 `dist/uploads/celeste` 是**空的**，是实打实的漏文件。）

所以现在用 `npm run deploy`（= `tools/deploy-pages.mjs`），它把顺序固定死：
工作区必须干净 → `pull --rebase` → `build` → **体检（产物里 uploads 的图片数必须与
git 仓库一致，缺一张就中止）** → 用唯一 hash 发布。
别再手敲 `wrangler pages deploy dist`。

顺带一条同源的经验：**「刚上传的图看不到」有两个独立原因**，别混为一谈：
1. 构建 + 传播延迟（约 1~2 分钟）—— 正常现象，图库现在用本机文件当缩略图顶过去；
2. 上面这个部署漏文件 —— 是事故，靠脚本里的体检拦住。

### 9.3 三个「用户能看到、但代码看着没问题」的坑（2026-09-19）

**① 登录表单在模块加载完之前会「原生提交」→ 页面刷新 → 表现为登录循环。**

`ui.js` 是 `type="module"`（延迟执行）。加载完成前表单没有监听器，
用户这时输口令按回车，浏览器就按表单默认行为提交（GET 带上 `?password=…`）→ 刷新 → 又回登录页。
**手机上网络慢时特别容易撞上**，而本地开发几乎遇不到（模块早就加载好了）。

修法：在表单后面加一小段**内联**脚本（立即执行、不延迟）先 `preventDefault()`，
模块加载好后再接真正的逻辑。**验证方式**也很关键 —— 用 CDP 的
`Network.setBlockedURLs` 把 `ui.js` 整个屏蔽掉，模拟「模块没加载」，再看提交会不会刷新。
（这一步一定要做：我第一次改完没屏蔽模块就测，测了个寂寞。）

**② Markdown 的单个换行不算换行 —— 用户会认为「我的换行丢了」。**

用户写俳句「礼貌启心门，/ 交谈舒畅似春风，/ 双赢喜相逢。」，三行在线上渲染成一行。
这是 Markdown 语义（单个换行合并成一段），不是 bug，但**在手机上写东西的人几乎都预期「回车=换行」**。

两条路各有代价：
- 开全局 `breaks`（GitHub 规则）：站里 13 篇含**软折行**的老文章会在句子中间凭空多出换行。
- **写的时候才转换**（最后选这条）：手机页保存时把单个换行写成 Markdown 硬换行（行尾两空格），
  只影响新写的那篇。围栏代码块里的换行必须跳过，否则会把代码改坏。

**③ 「至少 20 字」这种限制散落在四处 —— 去掉时要一起去掉。**

它同时写在 `tools/verify.mjs`、Worker 的 `/admin/save`、手机页的 `validate()`、
以及字数提示里。只改一处会出现「手机说能存、构建时被拦」。
去掉之后，测试要**反过来断言**（短正文/空正文必须放行），并且检查这几个文件里
都不再出现「长度 < N」的判断 —— 否则哪天有人顺手加回来，没有任何东西会报警。
